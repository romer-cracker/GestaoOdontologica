						...
						<div class="widget-content nopadding" >
							<form action="<%=request.getContextPath()%>/ServletUsuarioController" method="post" class="form-horizontal" id="formUser">
								...
								<div class="control-group">
									...
									<div class="controls">
										<input type="text" class="span11" name="nome" id="nome" required="required" value="${userAdminFuncionario.nome}">
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="admissao">Admissão :</label>
									<div class="controls">
										<input type="text" class="span11" name="admissao" id="admissao" required="required" value="${userAdminFuncionario.admissao}">
									</div>
								</div>
								<div class="control-group">
									<label class="control-label" for="login">login :</label>
									<div class="controls">
										<input type="text" class="span11" name="login" id="login" required="required" value="${userAdminFuncionario.login}">
									</div>
								</div>
								<div class="control-group">
									...
									<div class="controls">
										<input type="password" class="span11"
											 id="senha" name="senha" required="required" value="${userAdminFuncionario.senha}">
									</div>
								</div>
								...
								<c:forEach items='${admins}' var='adm'>
									<tr>
										...
										<td><a class="btn btn-success"
											href="<%= request.getContextPath() %>/ServletUsuarioController?acao=buscarEditar&id=${adm.id}" >Ver</a></td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
			...
			if (confirm('Deseja realmente excluir os dados?')) {
				...
				$.ajax({
					...
					success : function(response) {
						...
						function(xhr, status, errorThrown) {
							alert('Erro ao deletar paciente por id: '
									+ xhr.responseText);
						});
